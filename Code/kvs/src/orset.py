class ORSetFunctions:
    ...
    def display(payload):
        ...
        for item in payload.R:
            elems[item['elem']] = item['tags']
        qty_map = {}
        for item in payload.A:
            if item['elem'] in elems.keys():
                setA = set(item['tags'])
                setR = set(elems[item['elem']])
                quantity = len(setA.difference(setR))
                if quantity > 0:
                    qty_map[item['elem']] = quantity
            else:
                qty_map[item['elem']] = len(item['tags'])
        return qty_map
...
class ORSet():
    ...
    def display(self):
        ...
        """
        return self.orsetf.display(self)
